...
import { createMetricSchema, updateMetricSchema } from '@nl2sql/shared';
import { MetricService } from '../services/metric-service.js';
import type { DbClient } from '@nl2sql/db';

export function createMetricRouter(db: DbClient): Router {
  const router = new Router({ prefix: '/api/metrics' });
  const service = new MetricService(db);

  router.post('/', async (ctx) => {
    const parsed = createMetricSchema.safeParse(ctx.request.body);
    if (!parsed.success) {
      ctx.status = 400;
      ctx.body = {
        success: false,
    ...
    const metric = await service.create(parsed.data);
    ctx.status = 201;
  ...
  router.get('/:id', async (ctx) => {
    const metric = await service.getById(ctx.params.id);
    if (!metric) {
      ctx.status = 404;
      ctx.body = {
        success: false,
  ...
  router.patch('/:id', async (ctx) => {
    const parsed = updateMetricSchema.safeParse(ctx.request.body);
    if (!parsed.success) {
      ctx.status = 400;
      ctx.body = {
        success: false,
