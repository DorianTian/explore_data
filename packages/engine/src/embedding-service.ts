...
export class EmbeddingService {
  ...
  static buildAugmentedColumnText(params: {
    ...
    if (name.includes('amount') || name.includes('price') || name.includes('cost') ||
        comment.includes('金额') || comment.includes('价格') || comment.includes('费用')) {
    ...
    // Count / quantity
    if (name.includes('quantity') || name.includes('count') || name.includes('num') ||
        comment.includes('数量') || comment.includes('次数')) {
    ...
    if (dtype.includes('date') || dtype.includes('timestamp') || dtype.includes('time') ||
        comment.includes('日期') || comment.includes('时间')) {
    ...
    if (name.includes('category') || name.includes('type') || name.includes('status') ||
        comment.includes('分类') || comment.includes('类型') || comment.includes('状态')) {
    ...
    if (name.includes('name') || name.includes('title') ||
        comment.includes('名称') || comment.includes('名字')) {
      syntheticQuestions.push(`查找${params.columnComment ?? params.columnName}`);
    }

    // Channel / source
    if (name.includes('channel') || name.includes('source') ||
        comment.includes('渠道') || comment.includes('来源')) {
      syntheticQuestions.push(`各渠道对比`);
      syntheticQuestions.push(`不同${params.columnComment ?? params.columnName}的数据`);
    }

    // Region / location
    if (name.includes('region') || name.includes('city') || name.includes('area') ||
        comment.includes('地区') || comment.includes('城市') || comment.includes('区域')) {
