...
export function DetailTab() {
  ...
  /* Sync editedSql when selected message changes */
  useMemo(() => {
    if (message?.sql) {
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-2">
              ...
              {message.feedback && (
                <Badge
                  variant={
                    message.feedback === 'accepted' ? 'success' : 'error'
                  }
                >
                  {message.feedback === 'accepted' ? '已接受' : '已拒绝'}
                </Badge>
              )}
              {message.isGolden && <Badge variant="golden">Golden SQL</Badge>}
            </div>
          </div>
          <SqlEditor
            value={editedSql}
        ...
        <div>
          ...
          {message.executionResult.truncated && (
            <p className="text-xs text-muted mt-1">
              结果已截断，仅显示前{' '}
              {message.executionResult.rows.length} 行
            </p>
          )}
        </div>
      )}
